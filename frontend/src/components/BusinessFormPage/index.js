...
function BusinessPageForm() {
  ...
  const images = [
    ...
    "https://images.unsplash.com/photo-1591370874773-6702e8f12fd8?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1074&q=80",
  ];
  ...
  useEffect(() => {
    const error = [];
    if (title?.length < 1)
      error.push("You must put a title with at least 1 character");
    if (title?.length > 50)
      error.push("You must put a title with at max 50 characters");
    if (description?.length < 10)
      error.push("You must put at least 10 characters description");
    if (description?.length > 300)
      error.push("Description has a max 300 characters");
    if (address?.length < 5)
      error.push("You must put at 5 characters for a valid address");
    if (address?.length > 20) error.push("Address has a max 20 characters");
    if (state?.length < 1)
      error.push("You must put a state with at least 1 character");
    if (state?.length > 20) error.push("State has a max 20 characters");
    if (city?.length < 1)
      error.push("You must put a city with at least 1 character");
    if (city?.length > 36) error.push("City has a max 36 characters");
    if (zipCode?.length < 5 || zipCode?.length > 5)
      error.push("You must put a valid zipcode of 5 numbers max total");
    if (phoneNumber?.length !== 10)
      error.push("You must put 10 numbers for a valid phone number");
    if (image === "") error.push("Provide a picture");
  ...
  return (
    <form className="businessForm" onSubmit={onSubmit}>
      ...
      <div className="formDiv">
        ...
        {reveal ? (
          <div className="chosenImages-container">
            {images.map((item) => {
              return (
                <img
                  src={item}
      ...
      <button className="submitButtonBusiness" type="submit">
        Submit
      </button>
    </form>
  );
